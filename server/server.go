...
func NewServer(cfg *Config, manager oauth2.Manager) *Server {
	...
	srv.PasswordAuthorizationHandler = func(r *http.Request, clientID, username, password string) (string, error) {
		return "", errors.ErrAccessDenied
	}
	return srv
}
...
func (s *Server) ValidationTokenRequest(r *http.Request) (oauth2.GrantType, *oauth2.TokenGenerateRequest, error) {
	...
	case oauth2.PasswordCredentials:
		...
		userID, err := s.PasswordAuthorizationHandler(r, clientID, username, password)
		if err != nil {
			return "", nil, err
		} else if userID == "" {
